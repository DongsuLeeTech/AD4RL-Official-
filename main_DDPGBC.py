...
from Algos.DDPG_BC import DDPGBC
...
def main(args, log_dir, replay_buffer):
    ...
    policy = DDPGBC(args, num_inputs, num_actions, max_action, env.action_space, args.discount, args.tau)
    ...
    for ep in tqdm(range(args.epochs)):
        ...
        qf_tot, actor_tot = policy.train(replay_buffer, 1000000, args.batch_size)
        ...
        print('----------------------------------------------------------------------------------------')
        print('# epoch: {} # avg.reward: {} # cor.reward: {} # qf_loss: {} # actor_loss: {} '.format(ep, eval_reward,
                                                                                    correction_reward, qf_tot, actor_tot))
        ...
        summary.add_scalar('critic', qf_tot, ep)
...
if __name__=="__main__":
    ...
    for d in dataset_list:
        ...
        for seed in seed_list:
            ...
            log_dir_name = str(args.dataset) + '_Seed' + str(args.seed) + '_Batch' + str(args.batch_size)

            from datetime import datetime
            date = datetime.today().strftime("[%Y|%m|%d|%H:%M:%S]")
            log_dir = args.logdir + date + log_dir_name + 'DDPGBC'
            os.mkdir(log_dir)
