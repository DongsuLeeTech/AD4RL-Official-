...
'''
Implementation: https://github.com/sfujim/BCQ/blob/master/continuous_BCQ/BCQ.py
...
class VAE(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim, max_action, device):
        ...
        self.e1 = nn.Linear(state_dim + action_dim, 100)
        self.e2 = nn.Linear(100, 100)

        self.mean = nn.Linear(100, latent_dim)
        self.log_std = nn.Linear(100, latent_dim)

        self.d1 = nn.Linear(state_dim + latent_dim, 100)
        self.d2 = nn.Linear(100, 100)
        self.d3 = nn.Linear(100, action_dim)
...
class BCQ(object):
    def __init__(self,
                 args, state_dim, action_dim, max_action, device):

        latent_dim = action_dim * 2
        self.device = device
        self.phi = args.phi

        self.actor = Actor(state_dim, action_dim, max_action, self.phi).to(device)
    ...
    def select_action(self, state):
        with torch.no_grad():
            state = torch.FloatTensor(state).repeat(10, 1).to(self.device)
    ...
    def train(self, replay_buffer, iterations):
        tot_vae_loss, tot_critic_loss, tot_actor_loss = 0, 0, 0
        ...
        tot_vae_loss /= iterations
        tot_critic_loss /= iterations
        tot_actor_loss /= iterations

        return tot_vae_loss.detach().cpu().item(), tot_critic_loss.detach().cpu().item(),\
            tot_actor_loss.detach().cpu().item()

    def save(self, filename, ep):
        torch.save(self.actor.state_dict(), filename + f'{ep}' + "_actor")
        torch.save(self.critic.state_dict(), filename + f'{ep}' + "_critic")
        torch.save(self.actor_target.state_dict(), filename + f'{ep}' + "_actor_target")
        torch.save(self.critic_target.state_dict(), filename + f'{ep}' + "_critic_target")

    def load(self, filename, ep):
        self.actor.load_state_dict(torch.load(filename + f'{ep}' + "_actor", map_location=self.device))
        self.critic.load_state_dict(torch.load(filename + f'{ep}' + "_critic", map_location=self.device))
        self.actor_target.load_state_dict(torch.load(filename + f'{ep}' + "_actor_target", map_location=self.device))
