...
parser.add_argument('--seed', type=int, default=0,)  # bottleneck-random seed
...
parser.add_argument('--max-ts', type=int, default=3000)
...
def main(args, replay_buffer):
    ...
    replay_buffer.load4BC(f"./buffers/{buffer_name}")

    # Initialize and load policy and Q_net
    policy = BC(args, num_inputs, num_actions)
    done = True
    reward_list = []
...
if __name__=="__main__":
    seed_list = [0, 1, 2, 3]
    env_list = ['highway-final', 'highway-medium', 'highway-random', 'highway-final-medium',
                'highway-final-random', 'highway-humanlike', 'highway-ngsim']
    for i in env_list:
        args.dataset = i
